...
function eval_score_on_district(graph::BaseGraph,
                                ...
                                district::Int)::Number
    try
        attr = graph.flipped_attributes[score.key]
        return sum(attr[node]
...
function score_partition_from_proposal(graph::BaseGraph,
    ...
    #println(proposal.D₁, "\t", proposal.D₂, "\t", minimum(partition.dist_populations), "\t", maximum(partition.dist_populations))
